"""Deviation 61 (draft, 25 Sep 2026; analysis-only, to be adopted before the replication-01 data are read): four statistics
added to the Deviation 19 anomaly protocol, and the replication decision table of pre-flight 08 (Section 4 of
docs/preflight/08_paper1_replication_2026-09-21.md, restated unchanged by the 23 Sep reissue) with their roles in it. Each
statistic can only remove a path to "confirmed"; none can create one (``replication_decision`` checks this on every call).

(i)   Prediction-side uncertainty, folded into the replication z. A calibration term SE_cal = P (|ln F_gate| s_gate + |ln F_ro| s_ro),
      the first-order effect on the prediction of relative drifts s_gate of the gate error rates and s_ro of the observable qubits'
      readout errors (F_pred = P / P_noiseless of the same re-draw, F_ro = (a_i a_j)^2 its readout folding, F_gate = F_pred / F_ro;
      s_gate = the largest relative drift of four gate-rate classes and s_ro that of the readout errors across the committed
      calibration snapshots of the 7 days before the run, averaged over the placement's locations inside the cuts, ``rate_drift``),
      and a split-conformal
      dispersion factor phi = max(1, q), q the ceil((m + 1) x 0.6827)-th smallest |z| of the m unflagged tested grid points at
      L >= 4, the heavy-tailed depth class of both flags (``conformal_scores``, ``conformal_factor``):  z_r = (V_r - P) / (phi sqrt(SE_boot^2 + SE_pred^2 + SE_cal^2)). The simulation interval of the
      decision table's "explained by the calibrated model" row is widened by the same calibration term and the measured
      interval by phi.
(ii)  Grid-wide Holm. Two-sided p = erfc(|z| / sqrt 2) for every Deviation 19 single-point test of the Section 2 grid and every
      replication test, Holm step-down at family-wise level ALPHA_FW = 0.05 (``holm``). A flag is "firm" only if its adjusted
      p <= ALPHA_FW; a flag is confirmed only if its replication test's adjusted p <= ALPHA_FW as well.
(iii) Kappa inversion. kappa = 1 + ln R / ln F_gate, R = measured signal variance / P, F_gate = F_pred / (a_i a_j)^2 (the model's
      gate-noise attenuation with the readout folding of the level-0 observable removed); an excess per-layer error factor, not
      a kurtosis. Reported for each flag with the interval from the bootstrap interval of the signal variance, beside the
      grid-wide kappa_hat (random-effects fit of ln R = (kappa - 1) ln F_gate over the unflagged level-0 grid points with
      F_gate <= 0.95, ``grid_kappa``). A replicated flag consistent with a grid-wide excess (kappa_hat's interval above 1 and
      |z_kappa| <= 3) closes as a calibration effect.
(iv)  Per-draw regression at n = 19 / 20 (L = 4, the only flagged point with exact per-draw noiseless gradients): OLS slope b of
      the hardware gradients on the noiseless gradients of the same draws, b^2 against F_pred:
      z_dm = (b^2 - F_pred) / sqrt(SE(b^2)^2 + (F_pred x the (i) calibration term)^2), with the draw factor D = Var(g_noiseless) / P_noiseless
      and the paired attenuation A. The n20 flag is confirmed only if z_dm < -3 (a device deficit on the draws that ran);
      otherwise it closes as draw sampling.
...
CONFORMAL_MIN_M = 10                           # (i) fewer calibration points: phi not evaluable, no confirmation
...
READINGS = dict(
    opposite="opposite sign: not replicated; a new single-point flag (pre-flight 08 row 4)",
...
def holm_family(tests: pd.DataFrame, z_col: str = "z", alpha: float = ALPHA_FW) -> pd.DataFrame:
    """Grid-wide Holm (ii) on a family table with one row per Deviation 19 test (``point_id``, ``z_col``): adds ``p``,
    ``p_holm`` and ``holm_reject``; the family size is ``attrs['m']``. The table must hold every tested point, flagged or not."""
    out = tests.copy()
...
def conformal_factor(scores: Iterable[float], level: float = CONFORMAL_LEVEL, min_m: int = CONFORMAL_MIN_M) -> Dict:
    ...
    set (the unflagged tested grid points' Deviation 19 z). phi >= 1 by construction; not evaluable below ``min_m`` points."""
...
def calibration_rel_se(F_pred: float, F_ro: float, s_gate: float, s_ro: float) -> float:
    ...
    any correlation between the two drifts (F_gate = F_pred / F_ro)."""
...
def conformal_scores(grid: pd.DataFrame, z_col: str = "z", min_L: int = CONFORMAL_MIN_L) -> np.ndarray:
    """The (i) calibration set: the recorded Deviation 19 z of every tested, unflagged grid point with L >= ``min_L`` (columns
    ``L``, ``z_col``, ``flagged``; a flagged point's same-draw twin at the other resilience level counts as flagged)."""
...
def replication_z(V: float, se_boot: float, P: float, se_pred: float, se_cal: float = 0.0, phi: float = 1.0) -> Dict:
    """The pre-flight 08 statistic z_r = (V - P) / sqrt(SE_boot^2 + SE_pred^2) and its Deviation 61 (i) form
    ...
    s08 = math.sqrt(float(se_boot) ** 2 + se_pred ** 2)
    phi_ = max(1.0, float(phi)) if np.isfinite(_f(phi)) else float("nan")
    se_cal_ = float(se_cal) if np.isfinite(_f(se_cal)) else float("nan")
    s61 = phi_ * math.sqrt(float(se_boot) ** 2 + se_pred ** 2 + se_cal_ ** 2) if _finite(phi_, se_cal_) else float("nan")
    z08 = (float(V) - float(P)) / s08 if s08 > 0 else float("nan")
...
def rate_drift(snapshots: Sequence[str | Path], qubits: Iterable[int], couplers: Iterable[Tuple[int, int]], edge: Sequence[int],
    ...
    gate classes (CZ, sx, 1/T1, 1/T2: an upper bound on the first-order drift of ln F_gate for any split of the gate attenuation
    among them), ``s_ro`` = that of the observable qubits' readout errors. With fewer than ``min_snapshots`` distinct snapshots
    in the window the whole committed record before ``run_time`` is used (``window_extended``); with fewer still, the drifts are
    not evaluable (and no flag can be confirmed)."""
...
def per_draw_regression(g_hw, g_nl, shot_vars=None, P: float | None = None, P_noiseless: float | None = None, rel_se_cal: float = 0.0,
                        n_boot: int = N_BOOT, seed: int = SEED_PER_DRAW) -> Dict:
    ...
    ``rel_se_cal`` = ``calibration_rel_se`` of the point (the calibration term of (i) on the predicted attenuation)."""
...
# ------------------------------------------------------------------------------------------------ the family and the decision table

def point_z61(pt: Mapping, phi: float) -> Dict:
    """The Deviation 61 (i) z of one tested point from its record: ``V``, ``ci_lo``, ``ci_hi`` (raw variance and bootstrap
    interval), ``P``, ``se_pred``, ``P_noiseless``, ``a_i``, ``a_j``, ``s_gate``, ``s_ro``."""
...
def build_family(grid: pd.DataFrame, phi: float, replications: Sequence[Mapping] = ()) -> pd.DataFrame:
    """The (ii) family: one row per Deviation 19 test of the Section 2 grid (``point_id``, ``z`` = the recorded Deviation 19 z,
    ``flagged``) plus one per replication test. Flagged grid points carrying their record (``point_z61`` keys) and the
    replication tests get the full (i) z; every other point gets z / phi (its calibration term is left out, which can only
    lower its p-value and so never helps a flag in the step-down)."""
    rows = []
...
def replication_decision(rep: Mapping, family: pd.DataFrame | None = None, grid: Mapping | None = None) -> Dict:
    """Pre-flight 08's reading of one replicated point, with and without Deviation 61.

    ``rep`` keys: ``point_id`` (the replication test's id in ``family``), ``V`` (raw variance of the fresh draws), ``ci_lo``,
    ``ci_hi`` (its 95 percent bootstrap interval), ``P`` and ``se_pred`` (the replication-day re-draw row and its engine error),
    ``P_noiseless`` (noiseless row of the same re-draw), ``a_i``, ``a_j`` (level-0 readout folding; 1 at level 1), ``signal``,
    ``signal_lo``, ``signal_hi`` (shot-subtracted variance and interval), ``s_gate`` and ``s_ro`` (``rate_drift``), ``phi``
    (``conformal_factor``), ``sim`` = dict(P, se) (the calibrated noisy simulation on the replication-day calibration, with its
    own F_pred when it differs: ``P_noiseless``), ``per_draw`` (``per_draw_regression`` output, n20 flag only) and
    ``per_draw_required`` (True for the n19 / n20 flag). ``family`` is ``holm_family`` over every tested point including this
    one; ``grid`` is ``grid_kappa`` over the unflagged level-0 grid points.
    Returns the Deviation 61 reading, the pre-flight 08 reading and every condition with its value."""
    grid = grid or {}
    se_boot = (float(rep["ci_hi"]) - float(rep["ci_lo"])) / (2 * Z95)
    att = gate_attenuation(rep["P"], rep.get("P_noiseless", float("nan")), rep.get("a_i", 1.0), rep.get("a_j", 1.0))
    s_gate, s_ro = _f(rep.get("s_gate")), _f(rep.get("s_ro"))
    se_cal = calibration_se(rep["P"], att["F_pred"], att["F_ro"], s_gate, s_ro)
    zz = replication_z(rep["V"], se_boot, rep["P"], rep.get("se_pred", 0.0), se_cal, rep.get("phi", float("nan")))
    V = float(rep["V"])
    steps: List[Dict] = []
    ...
    level0 = int(rep.get("resilience_level", 0)) == 0
    if level0:
        kap = kappa_inversion(_f(rep.get("signal")) / float(rep["P"]), att["F_gate"],
                              _f(rep.get("signal_lo")) / float(rep["P"]), _f(rep.get("signal_hi")) / float(rep["P"]))
    ...
    kc = kappa_consistency(kap["ln_R"], kap["se_ln_R"], kap["ln_F"], grid) if grid and level0 else dict(z_kappa=float("nan"), consistent=False, grid_excess=bool(grid.get("excess", False)))
    ...
    pdr = rep.get("per_draw")
    if rep.get("per_draw_required") and level0:
        zdm = _f((pdr or {}).get("z_dm"))
        steps.append(dict(step="(iv) per-draw regression: b^2 against F_pred", value=zdm, threshold=f"< -{SIGMA:g} to confirm",
                          b2=(pdr or {}).get("b2"), F_pred=(pdr or {}).get("F_pred"), D=(pdr or {}).get("D")))
    ...
    if family is not None and len(family) and "p_holm" in family.columns:
        hit = family[family.point_id == rep.get("point_id")]
    ...
    if reading is None:
        reading = "confirmed"
    if reading == "confirmed" and r08 != "confirmed":      # Deviation 61 may only remove paths to "confirmed"
        raise AssertionError("Deviation 61 confirmed a flag that pre-flight 08 does not confirm")
    return dict(point_id=rep.get("point_id"), reading=reading, reading_text=READINGS[reading], confirmed=reading == "confirmed",
                reading_preflight08=r08, reading_preflight08_text=READINGS[r08], z=z, z_preflight08=z08, kappa=kap, steps=steps,
                decision_input=level0, note="" if level0 else "level 1 (same draws as level 0): reported beside the level-0 reading, not a decision input")
...
def main(argv=None) -> int:
    """``python -m gradvar.analysis.anomaly_stats decide spec.json``: spec = {"grid": [{"point_id", "z", "flagged", ...}, ...]
    (every Deviation 19 test of the Section 2 grid; flagged rows with their ``point_z61`` record), "calibration_z": [...]
    (the Deviation 19 z of the unflagged tested grid points at L >= 4), "grid_kappa_points": [{"ln_R", "se_ln_R", "ln_F"}, ...]
    (unflagged level-0 grid points with F_gate <= 0.95), "replications": [rep, ...] (``replication_decision`` keys)}.
    Prints one JSON document: phi, the family with Holm-adjusted p, kappa_hat and each replication reading."""
    ap = argparse.ArgumentParser(description="Deviation 61 statistics and the pre-flight 08 replication reading")
    sub = ap.add_subparsers(dest="cmd", required=True)
    d = sub.add_parser("decide")
    d.add_argument("spec")
    d.add_argument("--alpha", type=float, default=ALPHA_FW)
    a = ap.parse_args(argv)
    spec = _load(a.spec)
    conf = conformal_factor(spec.get("calibration_z", []))
    reps = [dict(r, phi=r.get("phi", conf["phi"])) for r in spec.get("replications", [])]
    fam = holm_family(build_family(pd.DataFrame(spec["grid"]), conf["phi"], reps), "z61", a.alpha)
    pts = pd.DataFrame(spec.get("grid_kappa_points", []))
    grid = grid_kappa(pts.ln_R, pts.se_ln_R, pts.ln_F) if len(pts) else {}
    out = dict(alpha=a.alpha, m=fam.attrs["m"], conformal=conf, grid_kappa=grid, family=fam.to_dict("records"),
               readings=[replication_decision(r, fam, grid) for r in reps])
