...
data/joblists/dryrun/03_paper2_smoke.json) from a calibration snapshot, with budgets from gradvar.hardware.estimate_budget.
    ...
    python scripts/make_paper2_joblists.py [--snapshot data/calibrations/ibm_phoenix_2026-09-19T155931Z.csv]

Every list is written with dry_run: true and the placeholder preflight_review; nothing here touches credentials.
Pre-registration: Paper 2 v0.4.2 (20 Sep 2026), Sections 2-3; Deviations 1-3.
"""
...
PREREG = "Paper 2 pre-registration v0.4.2 (20 Sep 2026)"
COMMON_NOTES = (" Execution (Section 3): SamplerV2 in job mode, resilience 0 (no twirling, no dynamical decoupling, no TREX term), "
                ...
                "{snap} snapshot: the 119 operational qubits (qubit 17 excluded everywhere), readout flags above 3e-2 "
                "{flags} and the CZ cluster {cz} flagged and kept (Section 3); the runner re-derives them from the named snapshot "
                ...
                "{m250} min at 250 us, {m1} min at 1 us. Nothing is submitted while dry_run is true and the pre-flight review "
                "permalink is the placeholder.")
...
def make_lists(snapshot: str) -> dict:
    ...
    q4_note = (f" Q4 patches (Section 3 rule, exclusion set = pre-registered cut plus Paper 1 Deviation 22 from the matching "
               f"properties file): {ctx.q4_edges}; the greedy row-by-row rule is infeasible at row 6 on this snapshot, so row 5 "
               f"takes its next-best edge ({q4_meta['backtracked']}), see docs/PAPER2_RUNNER.md.")
    lists = {}
    # Q1: 14 circuits with init_qubits True and the 3 arm-(b) circuits with init_qubits False, 65,536 shots (Section 3: 17 circuits, two jobs)
    lists["paper2/Q1.json"] = finish_notes(base(
        ctx, "paper2_Q1_reset_error_map", "Q1",
        f"{PREREG}, Section 2 Q1 / H1 and Section 3 'Order of runs' item 3: the reset-error map on all 119 operational qubits in "
        "parallel. Job Q1-init_true (init_qubits True, 14 circuits): (a) X -> reset kind -> measure and (b) reset kind -> measure "
        ...
        "pre-shot state. 65,536 shots per circuit, 1,114,112 executions (Section 3 table: 5.0 min at 250 us, 20 s at 1 us). Gate "
        "P2-2 must have passed on the smoke test before this list is submitted.",
        [dict(id="Q1-init_true", shots=65536, init_qubits=True, purpose="Section 2 Q1 arms (a)-(f), init_qubits True",
    ...
    lists["paper2/Q2.json"] = finish_notes(base(
        ...
        "stratified target masks M_s = {{q = 10 i + j : (i + 2 j) mod 5 = s}} (24 qubits each, no two within lattice distance 3, "
        "covering the 428 directed neighbour pairs once); neighbours and controls prepared in |+> for the X and Y readouts and in "
        "|0> for the Z (excitation) readout, the target in |0> or |1>; r in {{1, 4, 16}} repetitions of native reset or "
        "delay(400 ns; 2140 ns for target 79) on the targets while the rest idle; every operational qubit is read out. "
    ...
    lists["paper2/Q3.json"] = finish_notes(base(
        ...
        "cycles of [random Pauli on every qubit -> native reset on the mask, delay(400 ns) elsewhere -> the same Pauli]; reset "
        ...
        "four Pauli frames per mask, dense masks then frames drawn once from default_rng(20260919) (mask hashes in randomness); "
        "a circuit with m cycles uses the first m cycles of its 64-cycle frame. 156 circuits x 12,288 shots = 1,916,928 "
    ...
    lists["paper2/Q5.json"] = finish_notes(base(
        ...
        "(a), (b), (c) m = 2 and 4, (d) both, (e), (f) native; init_qubits True) at 32,768 shots, ONE day's job: dispatch this list "
    ...
    lists["dryrun/03_paper2_smoke.json"] = finish_notes(base(
        ...
        "qubit 79; three reset kinds as three distinct ISA instructions; readout within 1.5x of the snapshot; locked time per "
        "execution within 1.5x the Deviation 2 model) and by kill criteria 1-3. Converted from the EstimatorV2 <Z> estimate to "
        "SamplerV2 with init_qubits (the pre-registration's Section 3 'Execution' row), closing the Deviation the joblists README "
        "asked for. Composition (the pre-registration says 'about 40 circuits' without itemising them): job smoke-init_true (37 "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--snapshot", default=str(ROOT / "data" / "calibrations" / "ibm_phoenix_2026-09-19T155931Z.csv"))
