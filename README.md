...
python scripts/zz_layer_timing.py                                # Deviation 34: static ZZ time per coupler and layer from the scheduled ISA circuits
python scripts/gate1_pauliprop.py --stage gate1b --zz layer      # Deviation 34 (booked): static layer ZZ rzz(zeta tau_layer/2) + idle ZZ rzz(zeta 400ns/2)
python scripts/gate1_pauliprop.py --stage dev15 --zz layer --depths 8 12 --models unital nonunital
python scripts/pauliprop_zz_layer_validate.py                    # layer + idle ZZ vs the exact doubled-space theta average (2x3, L = 4, three models)
...
modelled there: T1 on the idle branch. `--zz layer` is the Deviation 34 model (booked): rzz(zeta tau_layer / 2) on every cone
coupler in every layer of the noisy models, tau_layer = 232-244 ns from the scheduled ISA circuits (`scripts/zz_layer_timing.py`),
plus the idle term rzz(zeta 400 ns / 2) in the dial layer (see `docs/PAULIPROP.md`, "Deviation 34"). Rules: unital depolarizing factors, T1/T2 relaxation (`Z -> (1-gamma) Z + gamma I`, `X, Y -> e^{-t/T2}`),
...
python -m gradvar.hardware --joblist data/joblists/paper1/dial_arm.json --dry-run-sample 8   # sampled dry run of a large list (first 8 pubs per job group)
```
...
The Paper 1 production lists (pre-registration v0.12.0: main grid per rung, Gate 1b references, reset-dial arm under the
Deviation 48 packing, Deviation 43 null controls, budgets under model v3) live under `data/joblists/paper1/`, generated by
`scripts/make_paper1_joblists.py` from the newest committed snapshot (Deviation 46) and documented with their budgets, order
of runs and remaining ambiguities in `docs/PAPER1_JOBLISTS.md`.
