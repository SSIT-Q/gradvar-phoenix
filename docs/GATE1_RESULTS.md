...
rows, 46 core-minutes. On this patch `interior_edge` gives 94_104, not the cone-graph-rule edge 93_103 that `grid_n40` measures, so the exact rows take the
run-day edge through the new `predict.hea_point(..., edge=)` override (recorded in the row's `edge_override`; the first run had skipped them). Exact rows:
L = 1 +0.0% (noiseless) / -1.1 to -1.4% (noisy); L = 2 k = 1 noiseless 7.70e-2 -> 1.02e-1 (+33%, M = 200 draw noise: the frozen and re-drawn intervals
overlap and the propagated value, 7.81e-2 -> 7.81e-2, does not move). Every propagated row falls: L = 2 k = 1 -0 to -1%, k = 2 -14 to -15%; L = 4 -24 to -34%; L = 8 -50 to -55%; L = 12 -59 to -67%
...
**n60** (6x10: frozen n = 53, origin (6, 0), edge 84_85 -> run-day n = 50, origin (3, 0), edge 43_44, broken 86_87 and 31_32; the same placement as the 03:08Z
