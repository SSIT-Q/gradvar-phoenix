...
| 20 Sep 2026 (Deviation 32) | ibm_phoenix smoke test, job list 02, about 0.66 Flex min modelled | jobs ran (10 of 10 completed on IBM, 52 QPU s reported by Owais's screenshot at 18:19 IST); results not yet in the repository; retrieval workflow on main since 92505b0, dispatch pending with Owais (Section 5) |
...
| This handover | https://claude.ai/artifact/97fbLq6XfiAYZ2Q3SKn4dq | Version 2, 20 Sep 2026 18:50 IST (Version 1 committed as 2b26c4f) | `docs/HANDOVER.md` | Edit the Markdown, re-render, republish at the same URL, commit |
...
|---|---|---|---|
| `main` | 2b26c4f / 92505b0 (20 Sep 13:16Z to 13:18Z); 3a8ceaa is the bot's smoke-run commit | everything merged through analysis-p1 (8bd7e8b, 10:08 IST) and pp-zz-idle (7155c34, 09:51 IST); the CI fix 884415e; Owais's arming commit 229a60f; the smoke-run snapshot (3a8ceaa); the retrieval path `--retrieve` / `--submit-only`, `retrieve_jobs.yml`, the ids file `data/runs/2026-09-20/smoke_02_job_ids.json` and `tests/test_retrieve.py` (92505b0, suite 132); this handover (2b26c4f) | the hardware runner on main already has `max_experiments` chunking and the L = 0 `null_control` (3a39250) |
...
Ten Estimator jobs in one Batch on ibm_phoenix, Flex instance, submitted 20 Sep about 10:15 IST (04:45Z) by run 35489912431: three grid jobs (n = 20 patch at origin (8,2), qubits 82 to 86 / 92 to 96 / 102 to 106 / 112 to 116, observable edge 94_104, coupler 95-96 broken; L = 2 and 8, k = 1, M = 10, 4096 shots, resilience 0 / 1 / 2), two dial probe jobs (reset p = 0.25 at L = 8 with 16 masks x 16 shots and its delay-matched control, at resilience 0 and 1), one reset-error job (prepare |1>, native reset, measure Z, 1024 shots), and four rep_delay ladder jobs (prepare |0> / |1> then measure, 4096 shots, at 1 / 5 / 20 / 250 us). Budget model v2: 39.6 s at 1 us; observed about 52 QPU s (per Owais's platform screenshot; confirm from `job.usage()`). The IBM job ids are not in the Action log (buffered stdout); `--retrieve data/runs/2026-09-20/smoke_02_job_ids.json` (workflow "retrieve hardware jobs", landed 92505b0) discovers the ten jobs by signature in the 04:45 to 10:46Z window, fills the ids into the file, and writes the bundles under `data/runs/2026-09-20/<job_id>/` and the CSV under `data/jobs/`, exactly as the live path would have. If discovery finds more or fewer than ten matching jobs it stops and says which signature is ambiguous; then paste the ids from the IBM platform into the file's `job_id` fields and re-dispatch.
...
| 18:50 (13:20Z) | Handover updated for the retrieval workflow (Version 2) | Claude | `docs/HANDOVER.md`, this commit |

## 10. Open items and next steps, in order

1. Smoke-test retrieval: the workflow is on `main` (92505b0); Owais dispatches "retrieve hardware jobs" with `ids_file = data/runs/2026-09-20/smoke_02_job_ids.json` (asked 18:46 IST). It writes the ten bundles under `data/runs/2026-09-20/<job_id>/`, the CSV under `data/jobs/` and the filled ids file, committed by the bot. Do not poll; wait for Owais's post, then pull `main`. If discovery fails (Section 5), paste the ids from the IBM platform into the file and re-dispatch.
