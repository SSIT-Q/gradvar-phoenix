...
| P1.1.20 | Whole-layer ZZ in both propagation models (Deviation 34) | Cl | 20 to 22 Sep 2026 | in progress | In progress 20 Sep 2026 on branch pp-zz-layer: per-edge static ZZ over the whole layer, convention exp(−iζτ/4 ZZ) with weight sin²(ζτ/2), in both the unital and the non-unital propagation model; Gate 1 report rows recomputed on merge
...
| P1.2.2 | ibm_phoenix smoke test, about 5 Flex minutes: locked time per point, reset error and latency, depth ceiling, and the rep_delay ladder (P1.2.8) | O | 20 Sep 2026 (Deviation 32) | waiting: arming step denied by the permission system 09:21 IST; Owais to set preflight_review + dry_run false and dispatch run_jobs.yml | Waiting since 09:21 IST 20 Sep 2026: pre-flight for list 02 posted in the thread 09:16 IST (ts 1789874970.224149); the arming step (preflight_review permalink + dry_run false on list 02, then dispatch of run_jobs.yml) was denied by the permission system as a production deploy, so Owais sets the two fields, commits and dispatches run_jobs.yml himself; nothing spent yet. Post-run review as P1.2.14. Earlier: In progress 20 Sep 2026 (Deviation 32: smoke test brought forward from 27 to 29 Sep by Owais, 08:24 IST). Dry-run list 02, about 0.66 Flex minutes at the 1 µs default. Patch shifted one column to 82–86 / 92–96 / 102–106 / 112–116, observable edge 94_104, after Q91's initialisation error (1.05e-3) rose above the 5e-4 cut on the 03:05 UTC snapshot; coupler 95–96 broken (a Section 2 logged shift, not a deviation; runner now places from the raw properties at build time). Smoke-test summary in the thread; raw results committed
...
| P1.2.13 | Paper 1 production job lists (null_control points, 16384-shot references) | Cl | 26 Sep 2026 | todo | Production job lists with tests on main: null_control point type (Deviation 43), the three 16384-shot depth-8 references (Deviation 44), cost-cut of noisy points (P1.1.17); pre-flight review link in each list |
| P1.2.14 | Post-run review of the ibm_phoenix smoke test (list 02) | Cl reviews | After the run | todo | Post-run review file indexed in docs/REVIEWS.md; job ids, usage, locked time, reset error, rep_delay ladder and the Gate 2 reading recorded in P1.2.2 and the ledger (P1.2.3) |
...
| Reserve (itemised below: dry run, anomaly replications, Deviation 44 references, M-rule, null control, unallocated) | about 14 | 50 | 0 | 50 | 20 Sep 2026 |
