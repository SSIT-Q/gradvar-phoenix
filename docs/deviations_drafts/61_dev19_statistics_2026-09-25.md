...
- Family (`decision_family`), m = 47:
  ...
  - the four replication tests (`REPLICATION_TESTS`): replication_01 n20 L4 at levels 0 and 1; replication_01 n100 L8 level 0 at
    ...
    p = 1, so m stays 47. A renamed or re-placed list is not one of these tests and has to be named before its data are read.
...
figures are provisional: the draws are synthetic, the tail model is assumed, and the replication-side drifts stop at 25 Sep 03:08Z
rather than the replication-day snapshot.
