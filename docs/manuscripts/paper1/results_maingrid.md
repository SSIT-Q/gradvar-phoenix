...
`docs/postrun/03_paper1_day1_2026-09-20.md` and `04_paper1_day2_2026-09-20.md`. Nothing here is a finding beyond those two days' data; every rung is compared against its run-day re-draw (Deviations 46, 54; the n = 85
re-draw landed after the day-2 review, b5da7e5), one level-2 point is short of draws (the failed job L2-c5), and two Deviation 19 flags
stand (n = 19, L = 4; n = 85, L = 8), both booked for replication. Numbers are variances of the parameter-shift gradient of ⟨Z_i Z_j⟩ on the patch
...
| item | value |
|---|---|
| pre-registration | v0.14.1 (Deviations 14–55); v0.14.2 records the second Deviation 19 flag; this draft reads Sections 3–5 under Deviations 37, 40, 42, 43, 52, 54 |
...
| predictions, run-day re-draws (Deviations 46, 54) | n = 19: `main_grid_redraw_2026-09-20T1344.csv` (c84ecde); n = 37: `..._1750_n40.csv` (1cce692; propagation rows 8e74e67); n = 50: `..._n60.csv` (3953fc5); n = 69: `..._n80.csv` (85ac3c0); n = 85: `..._n100.csv` (b5da7e5; its L = 12 noisy rows are sampler-capped lower bounds) |
| predictions, 19 Sep frozen rows (record; used only for the L = 1 rows, whose two-qubit cone is placement-free) | `gate1_predictions.csv` (eece115, n = 20 / 87), `pauliprop_predictions.csv` (e3859fd) |
...
the resubmitted L2-c5 point supplies the second repeat (H4 part 2), and the two flagged points until replicated (Deviation 19: n = 19, L = 4; n = 85, L = 8).
...
the 10×10 patch (run-day re-draw, 0.01193) agree to 1 percent, so the factor-2 refutation clause does not fire; the measured level-0 values on those
...
| 2 | 0.0816 → 0.0783 (×0.96) | −0.00 ± 0.11 | +0.05 | flat |
| 4 | 0.00578 → 0.00816 (×1.41) | +0.21 ± 0.14 | +0.06 | flat within 1.5 σ; the n = 19 point is the flagged low draw (below) |
| 8 | 3.03e-4 → 2.44e-4 (×0.81) | +0.03 ± 0.17 (weighted by the intervals: −0.14 ± 0.15, pulled by the precise 16384-shot point; on the signal: +0.01 ± 0.29) | +0.17 | flat |
...
L ≤ 8 on the claimable points; the model's own α is +0.02 to +0.17 from cone and broken-coupler differences between placements, not from n.
...
measured 95 percent interval and 36 have |z| ≤ 2; ratios run 0.50 to 1.60. At L ≤ 4 (30 points) the ratios are 0.61 to 1.36 and, apart from the
...
at 16384 shots reads 0.50 [0.33, 0.71] of its run-day row 3.07e-4 (z = −5.2 raw, −6.2 signal; level 1 on the same draws: 0.62, z = −2.8) while
the 4096-shot level-1 point of the same rung on other seeds reads 1.30 (0.90 in the signal). The Deviation 19 single-point flag fires on two
points: n = 19, L = 4 (both levels) and n = 85, L = 8 at 16384 shots (level 0), both recorded in the pre-registration (v0.13.3, v0.14.2) and booked
for replication. No monotone same-sign run over three adjacent n or L (Deviation 19 (ii), Deviation 42 (viii)). The same-draw
...
| 85 | 8 | 4.00e-4 [2.74e-4, 5.44e-4] | 6.41e-4 (kurtosis 27) | 4.08e-4 | 7.51e-4 [4.97e-4, 1.05e-3] | 1.88 [1.24, 2.80] | 2.26 [1.31, 4.00] | 1.49 (1.51), run-day row |
| 37 | 12 (expl.) | 1.21e-4 | 1.37e-4 | 1.75e-4 | 1.52e-4 | 1.44 [1.04, 1.96] | at the shot floor | 1.96 (1.92) |
| 85 | 12 (expl.) | 1.53e-4 | 1.40e-4 | 1.49e-4 | 1.54e-4 | 0.91 [0.67, 1.24] | at the shot floor | 1.47 (1.66; run-day rows are sampler-capped lower bounds) |
...
1.7 (1.67 at n = 37, 1.49 at n = 85); the k = L/2 and k = L−1 points sit at the k = 1 level within their intervals (n = 85, k = 4 excepted, a kurtosis-27 draw set). The
...
gives 2.6 [1.3, 5.2] at n = 85 by hand); this remains a consistency reading until the pipeline is re-keyed to the placed n. At L = 12
...
against 0.0783 and 0.0711 (run-day exact row). Mitigation recovers none of the L = 2 attenuation (the same-draw noiseless variances are 0.0685 and 0.0869,
postrun 04). n = 37, L = 8: 6.29e-4 [4.27e-4, 9.24e-4] against level 0 3.24e-4 (1.9×) and the prediction 2.38e-4; n = 85, L = 8: 7.04e-4
[3.78e-4, 1.08e-3] on **100 draws** against the level-1 4.00e-4 and the prediction 3.07e-4. With the pre-registered level-2 shot floor 4 × 1/(2N) =
...
1.41e-4 and 2.16e-4 (z = −0.8 and −0.5 at L = 8). Caveat, stated as pre-registered: ZNE inflates the shot variance by the extrapolator's ‖c‖₁²,
...
2. **n = 85, L = 8, k = 1, level 0, 16384 shots (day 2): flagged** (pre-registration v0.14.2 status record). 1.53e-4 [1.02e-4, 2.17e-4] against
   the run-day non-unital row 3.07e-4 ± 0.09e-4 (b5da7e5; the 19 Sep frozen row was 2.90e-4, so the re-draw moved the prediction up 6 percent and
   the flag stands): ratio 0.50 (0.40 in the signal), z = −5.2 raw / −6.2 signal; level 1 on the same draws 1.91e-4, ratio 0.62, z = −2.8. The
   4096-shot level-1 point of the same rung on other seeds (24391xxx) reads 1.30 of the same row (0.90 in the signal), so the two seed sets differ
   by about 1.7× as on day 1 (kurtosis 8.7 and 7.2). No same-draw noiseless reference exists at this point (the L = 8 cone covers the whole 85-qubit
   patch), so the draw-sampling and attenuation factors cannot be separated as they were at n = 19; the other L = 8 rungs read 1.24, 1.36, 1.18 and
   1.60 in the raw variance (0.74 to 1.19 in the signal). The point joins the replication list (P1.3.9, fresh seeds, another day and patch) and is
   exploratory until then.
...
4. **Exploratory by pre-registration:** all L = 12 rows (signal 3e-6 to 4e-5 against predictions 7e-6 to 1.5e-5, at the 1.22e-4 shot floor; the
   propagation rows at L = 12 are marked not converged); the H3 sweep; the level-2 rows until L2-c5 is resubmitted. Hardware-only points (Deviation 15): none on the main grid; the n = 19 and n = 50 L = 8 propagation rows are marked not
   converged and carry their truncation error in σ; the n = 85 L = 12 noisy rows of the run-day re-draw are sampler-capped lower bounds (used as
   drawn, without σ).
...
- Fig. 1 `fig1_variance_vs_L_per_n`: Var[∂C] against L, one panel per rung (n = 19, 37, 50, 69, 85), level 0 with 95 percent intervals; non-unital
  model (run-day re-draw on every rung; the L = 1 rows are the placement-free 19 Sep record), noiseless model, the shot floor, the
...
- Fig. 3 `fig3_pull_hardware_vs_prediction`: measured / predicted variance (top) and z (bottom) per point, levels 0 and 1, grouped by L; L = 12 as signal / prediction, exploratory.
