"""Paper 2 (pre-registration v0.4.2): the SamplerV2 path of the runner, the Q1-Q5 circuit builders, the qubit-set and mask
selection, the Sampler budget and the per-shot register capture. Everything runs against FakeNighthawk / Aer; nothing
...
# Section 3 table: (circuits, jobs, executions, minutes at 250 us, seconds at 1 us); Q5 is one of four days
TABLE = {"Q1": (17, 2, 1_114_112, 5.0, 20), "Q2": (180, 1, 2_949_120, 13.1, 49), "Q3": (156, 1, 1_916_928, 8.8, 52),
         "Q4": (384, 1, 786_432, 3.5, 13), "Q5": (8, 1, 262_144, 4.7 / 4, 23 / 4),
         # the table budgets the smoke test as one job (3 s at 1 us); list 03 runs two (init_qubits True / False) and pays a
         # second 2 s job charge: 5.3 s at 1 us, 0.43 min at 250 us (docs/PAPER2_RUNNER.md, ambiguity 3)
         "smoke": (40, 2, 81_920, 0.4, 3 + 2.3)}
...
def test_lists_validate_and_refuse_to_submit(name, tmp_path, monkeypatch):
    ...
    assert "pre-registration v0.4.2" in jl["notes"] and jl["protocol"] == name
    assert check_budget(jl) == [] and jl["budget"]["model_version"] == 2 and jl["budget"]["primitive"] == "sampler"
    assert jl["qubit_set"]["snapshot"] == "ibm_phoenix_2026-09-19T155931Z.csv" and len(jl["qubit_set"]["qubits"]) == 119
    assert 17 not in jl["qubit_set"]["qubits"] and jl["qubit_set"]["flagged_readout"] == [24, 49, 55, 62, 73, 77, 107]
...
def test_budgets_match_the_preregistration_table():
    """Section 3 'Minute budget' and Deviation 2: Sampler at resilience 0, no TREX; 35.4 min at 250 us, 2.7 min at 1 us, cap 45."""
    ...
    for name, (circuits, jobs, execs, minutes, seconds_1us) in TABLE.items():
        ...
        assert b["seconds_at_1us"] == pytest.approx(seconds_1us, abs=1.0), name
    ...
    assert total_250 == pytest.approx(35.4, abs=0.15) and total_250 <= 45.0
    assert total_1 == pytest.approx(2.7, abs=0.1)
    ...
    assert per["Q1-init_true"]["init_qubits"] is True and per["Q1-init_false"]["init_qubits"] is False
    assert per["Q1-init_true"]["circuits"] == 14 and per["Q1-init_false"]["circuits"] == 3
    assert jl["budget"]["mcm_executions"] == 8 * 65536                         # a, b, f x measure_reset(_2) + arm (b) again
    for job in jl["sampler_jobs"]:
        lengths = [circuit_gate_us(c, DIAL_US) for c in expand_job(job, ctx)]
    ...
    assert circuit_gate_us(dict(kind="q3", cycles=64), DIAL_US) == (pytest.approx(0.08 + 64 * 0.48), 0)   # 30.8 us (Section 3: 30.7)
...
def test_qubit_sets_masks_and_patches():
    ...
    assert len(ops["qubits"]) == 119 and ops["excluded"] == [17] and "no T1 or T2" in ops["reasons"]["17"]
    assert ops["flagged_readout"] == [24, 49, 55, 62, 73, 77, 107] and ops["flagged_cz_cluster"] == [55, 61, 62, 63, 72, 73]
    masks = p2.q2_masks()
    assert [len(m) for m in masks] == [24] * 5 and sorted(q for m in masks for q in m) == list(range(120))
    for m in masks:
    ...
    assert len(covered) == 428 and len(set(covered)) == 428                    # all directed neighbour pairs once
    r0 = p2.q2_roles(masks[0], ops["qubits"], ops["flagged_readout"])
    assert 17 not in r0["targets"] and len(r0["targets"]) == 23 and set(r0["controls"]) >= {7, 16, 18, 27}   # 17's neighbours idle as controls
    ...
    assert len(dense) == 8 and 0.35 < np.mean([len(m) / 119 for m in dense]) < 0.65 and frames.shape == (4, 64, 119)
    ...
    assert list(table) == [f"sparse{s}" for s in range(5)] + [f"dense{i}" for i in range(8)] and len(table["sparse0"]) == 23
    ...
    assert set(ex) >= {17, 24, 49, 55, 61, 62, 63, 72, 73, 77, 107} and {8, 11, 18, 22, 27, 59} <= set(ex)   # Deviation 22 additions
    ...
    assert edges["edges"] == [[0, 1], [15, 16], [20, 21], [38, 39], [42, 43], [57, 58], [64, 65], [78, 79], [80, 81], [97, 98], [100, 101], [115, 116]]
    ctx = p2.SamplerContext.from_snapshot(CAL)
    assert ctx.q4_edges == edges["edges"] and ctx.qubits == ops["qubits"]
...
def test_dry_run_every_list(name, tmp_path):
    ...
    for job in jl["sampler_jobs"]:
        d = bundles[job["id"]]
        assert {p.name for p in d.iterdir()} == {"job.json", "result.json", "metadata.json", "options.json", "properties.json", "target.json", "circuits.qpy", "circuits.json"}
        ...
        assert j["qubit_set"]["qubits"] == jl["qubit_set"]["qubits"] and j["q4_patches"] == jl["q4_patches"]["edges"]
        ...
        for pt, c in zip(j["points"], circs):
            ...
            if pt["reset_kind"] in ("measure_reset", "measure_reset_2"):
                assert pt["synthetic_target_instructions"] == [pt["reset_kind"]] and "mcm" in pt["registers"] and len(pt["registers"]["mcm"]) == 119
                assert c["ops"][pt["reset_kind"]] == 119 and pt["sched_ns"] is None        # synthetic stand-in has no duration
            else:
            ...
            if pt["kind"] == "q2" and pt["reset_kind"] == "delay":
                assert c["delay_count"] == pt["reps"] * len(pt["targets"]) and c["delay_durations_ns"] == [400.0] + ([2140.0] if 79 in pt["targets"] else [])
...
def test_simulated_dry_run_captures_every_register(tmp_path):
    ...
    assert len(mcm_pubs) == 6 and all(f"pub{i}__mcm" in arrays and arrays[f"pub{i}__mcm__num_bits"][0] == 119 for i in mcm_pubs)
    assert all(f"pub{i}__mcm" not in arrays for i in range(37) if i not in mcm_pubs)
    assert arrays["pub0__meas"].shape == (64, 15) and arrays["pub0__meas"].dtype == np.uint8 and arrays["pub0__meas__num_bits"][0] == 119
    r = by_label["Q1a_reset"]["meas"]
    assert r["num_shots"] == 64 and r["num_bits"] == 119 and r["qubits"] == ctx.qubits and r["all_zero_fraction"] == 1.0 and max(r["p1_per_bit"]) == 0
    assert min(by_label["Q1d_x_measure"]["meas"]["p1_per_bit"]) == 1.0 and min(by_label["Q1e_x_delay_measure"]["meas"]["p1_per_bit"]) == 1.0
    assert max(by_label["Q1d_measure"]["meas"]["p1_per_bit"]) == 0 and by_label["Q1d_measure"]["meas"]["counts"] == {"0" * 119: 64}
    ...
    assert 0.35 < np.mean(f) < 0.65 and f != [0.0] * 119                                 # |+> -> reset -> |0>, read in X: <X> = 0 (no residual coherence)
...
def test_submit_path_under_mock_writes_bundles_and_applies_the_paper2_layout_policy(tmp_path, monkeypatch):
    ...
    arrays = np.load(d / "bitarrays.npz")
    assert arrays["pub0__meas"].shape == (32768, 15) and "pub0__mcm" not in arrays        # Q5 is native reset only
    o = json.loads((d / "options.json").read_text())
