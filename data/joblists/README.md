...
| `layout_check` | optional: `"enforce"` (the default behaviour) or `"override"`. Before submitting, the runner re-checks every layout qubit and coupler of the built pubs against the **live** `backend.properties()` (not the snapshot the list was built from): readout assignment error above the pre-registered cut `gradvar.noise.READOUT_CUT` (3e-2), a non-operational qubit, or a missing readout figure fails the qubit; a CZ error above `gradvar.hardware.CZ_CUT` (1e-2, five times the 19 Sep ibm_phoenix median; not a pre-registered number, record it as a Deviation before the grid) or an uncalibrated pair fails the coupler. The per-qubit / per-coupler figures, the failing items and the verdict (`pass` / `fail` / `unavailable`) are written to every `job.json` as `layout_check`. On anything but `pass` the submitting path refuses (before any job is created) unless the list carries `layout_check: "override"` with a non-empty `layout_check_reason`, which is then logged in `layout_check.override`. Dry runs against fake backends (stale calibrations) only log the check (`enforced: false`). For ibm_phoenix lists `place_patch` already applies the readout cut at build time from the calibration CSV; the live re-check still logs and enforces. Review defect D2: Q11 (readout 0.084) sat in the Marrakesh path unnoticed |
| `layout_check_reason` | required with `layout_check: "override"`: why a failing qubit or coupler is acceptable for this list (for example a qubit that the observable does not read out) |
